...
|---|---|---|
| Quality / model behavior | Is the system behaving correctly? | input guardrail accuracy, item ID quality, schema validity, safety pass rate |
| User impact | Is the product useful and trusted? | correction rate, retake completion rate, repeat usage, feedback sentiment |
| Business / operational outcomes | Is the system viable to run? | cost/request, p95 latency, incident rate, pilot retention, support burden |
...
- **Bullets:**
  - Helps users managing high cholesterol track saturated fat from meal photos
  - Turns a hard manual habit into a lightweight daily workflow
  - Creates immediate user value without requiring a full nutrition platform
- **Speaker notes:** Emphasize that the product is useful because it is specific. We are solving one meaningful tracking problem first.
...
### Slide 10 — Evaluation
- **Bullets:**
  - Offline eval before launch: food detection, item ID, sat-fat estimate quality, schema validity, safety
  - Post-launch monitoring: latency, cost, rejection rates, corrections, incidents
  - Three layers: quality, user impact, operations
- **Speaker notes:** Explain that evals make iteration disciplined rather than subjective.
...
## SECTION K — Loom Script

Hello everyone. In this walkthrough, I’m going to show a production-ready design for an AI-powered Meal Analysis Cholesterol App, with a very intentional Phase 1 scope.

We are not trying to solve general nutrition from day one. We are starting with one narrow, high-value use case: helping people who are managing high cholesterol estimate saturated fat from a meal image and track their saturated fat across the day.

That narrowness is a strength. It gives us a clearer user promise, a smaller safety surface, and a system we can actually evaluate before launch. Instead of claiming exact nutrition truth from an image, the product provides bounded, uncertainty-aware estimates that are useful for tracking behavior.

The core user flow is simple. A user uploads a meal photo from the mobile app. The system validates that the image is usable and actually contains food. If the image passes, the system analyzes the meal, identifies likely food items, estimates saturated fat, and then updates the user’s daily saturated fat total. Before anything is returned, output guardrails make sure the final response stays informational and avoids medical advice.

The most important design decision is to use a bounded workflow instead of a single giant model call. In production, we want separate components for input guardrails, meal inference, output guardrails, state tracking, and monitoring. That gives us clearer failure isolation. If the system is rejecting too many images, we know it is an input problem. If outputs are overconfident, we know where to tighten prompts or templates. If daily totals drift, that is a state management issue, not a model mystery.

At the architecture level, the mobile app handles capture, guidance, and display. The API orchestrator coordinates the workflow and enforces idempotency and timeouts. Input guardrails reject non-food or unusable images early. The meal inference service uses a stronger multimodal model only after the image is accepted. Output guardrails keep the language safe, bounded, and clearly non-medical. Then the user state service updates the running daily total. Across all of this, the logging and evaluation layer records latency, cost, confidence, failures, and safety outcomes.

Model strategy is tiered. We do not want one expensive model doing everything. Cheap deterministic checks should handle upload integrity and some quality rules. A lightweight classification step can help with food versus non-food routing. Then a stronger multimodal model handles the core reasoning task: what food is visible, what portion cues exist, and what a reasonable saturated fat estimate might be. Finally, user-facing wording should be templated or lightly reviewed so we do not accidentally drift into medical advice.

Structured outputs are essential here. We need predictable fields like request ID, food items, saturated fat estimate, confidence level, ambiguity flags, daily total, disclaimers, and an escalation flag. This supports state updates, monitoring, and eval automation. It also prevents the model from hiding uncertainty in vague prose.

Confidence matters a lot in this product. There will be many cases where an image alone is not enough: creamy pasta, mixed dishes, hidden oils, restaurant meals with unclear portions. A trustworthy product should be comfortable saying, “I can identify the meal, but I cannot estimate saturated fat reliably from this image alone.” That is better than fake precision.

For safety, this system should be explicit about what it does not do. It does not diagnose high cholesterol. It does not prescribe treatment. It does not replace a clinician or dietitian. It provides meal-level estimates for tracking purposes only. That policy should be enforced in prompts, output templates, QA checks, and incident review.

Evaluation should begin before launch. We need an offline dataset with real food images, mixed meals, packaged foods, low-light photos, non-food uploads, and difficult portion cases. We should measure input validation quality, meal item identification, saturated fat estimate quality, schema correctness, and policy compliance. After launch, we monitor latency, cost per request, rejection rates, low-confidence rates, user correction rates, and safety incidents.

Finally, on team workflow: Codex can accelerate development significantly, especially for scaffolding routes, generating typed clients, writing eval scripts, debugging schema failures, and drafting documentation. But it does not replace engineering judgment. Prompts and schemas should be versioned. Evals should exist before scaling. And code review plus safety review remain mandatory.

The main takeaway is this: if we start narrow, use a bounded architecture, and build around structured outputs, guardrails, and evaluation, we can deliver a system that is realistic, safe, measurable, and expandable. That is the right way to launch this kind of health-adjacent AI product in production.
...
```dotenv
OPENAI_API_KEY=
OPENAI_MODEL_MEAL_INFERENCE=
OPENAI_MODEL_INPUT_GUARDRAILS=
OPENAI_MODEL_OUTPUT_REVIEW=
DATABASE_URL=
